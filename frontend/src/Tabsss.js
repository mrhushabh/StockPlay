    ...
    useEffect(() => {
        const fetchdata = async (assignSymbol) => {
            try {
                ...
                const response4 = await axios.get('/api/News', {
                    params: {
                        symbol: props.ticker
                    }
                });

                const response6 = await axios.get('/api/Chart1', {
                    params: {
                        symbol: props.ticker
                    }
                });
                const response7 = await axios.get('/api/Chart2', {
                    params: {
                        symbol: props.ticker
                    }
                });
                const response8 = await axios.get('/api/Chart4', {
                    params: {
                        symbol: props.ticker
                    }
                });

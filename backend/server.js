...
const dbUrl = "mongodb+srv://madurwar_db_user:Stocksearch%40123@cluster0.pwgklsi.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0"
...
app.post('/api/addfav', async (req, res) => {
  try {
    const { stockName, symbol, price, change, percentchange } = req.body;
    let existing = await Fav.findOne({ stockName });
    if (existing) {
      console.log('Do nothing')
    }
    else {
      const newfav = new Fav({ stockName, symbol, price, change, percentchange });
      await newfav.save();
      res.json(newfav);
    }
  } catch (error) {
...
app.post('/api/star', async (req, res) => {
  try {
    ...
    const stardata = await Fav.findOne({ stockName });
...
app.get('/api/company', async (req, res) => {
  try {
    ...
    console.log(symbol)
    const response = await axios.get(`https://finnhub.io/api/v1/stock/profile2?symbol=${encodeURIComponent(symbol.symbol)}&token=cmundmpr01qltmc10l9gcmundmpr01qltmc10la0`);
    const data = response.data;
    console.log(data);
    res.json(data);
  } catch (error) {
...
app.get('/api/quote', async (req, res) => {
  try {
    ...
    console.log(symbol)
    const response = await axios.get(`https://finnhub.io/api/v1/quote?symbol=${encodeURIComponent(symbol)}&token=cmundmpr01qltmc10l9gcmundmpr01qltmc10la0`);
    const data = response.data
    console.log(data);
    res.json(data);
  } catch (error) {
...
app.get('/api/peers', async (req, res) => {
  try {
    ...
    console.log(symbol)
    const response = await axios.get(`https://finnhub.io/api/v1/stock/peers?symbol=${encodeURIComponent(symbol)}&token=cmundmpr01qltmc10l9gcmundmpr01qltmc10la0`);
    const data = response.data
    console.log(data);
    res.json(data);
  } catch (error) {
...
app.get('/api/News', async (req, res) => {
  try {
    ...
    console.log(symbol)
    const response = await axios.get(`https://finnhub.io/api/v1/company-news?symbol=${encodeURIComponent(symbol)}&from=2023-08-15&to=2023-08-20&token=cmundmpr01qltmc10l9gcmundmpr01qltmc10la0`);
    const data = response.data
    console.log(data);
    res.json(data);
  } catch (error) {
...
app.get('/api/Sentiments', async (req, res) => {
  try {
    const symbol = req.query; // Access symbol from query parameters

    console.log(symbol);
    const response = await axios.get(`https://finnhub.io/api/v1/stock/insider-sentiment?symbol=${encodeURIComponent(symbol.symbol)}&token=cmundmpr01qltmc10l9gcmundmpr01qltmc10la0`);
    const data = response.data;
    console.log(data);
    res.json(data);
  } catch (error) {
...
app.get('/api/Chart1', async (req, res) => {
  try {
    ...
    console.log(symbol);
    console.log('hee');
    const response = await axios.get(`https://api.polygon.io/v2/aggs/ticker/${encodeURIComponent(symbol.symbol)}/range/1/day/2023-09-14/2024-03-17?adjusted=true&sort=asc&apiKey=OpXGdu_iIjhzVRAXnby_ktJX7iHAurj9`);
  ...
  try {
    ...
    const response = await axios.get(`https://finnhub.io/api/v1/stock/recommendation?`, {
      params: {
        ...
        token: 'cmundmpr01qltmc10l9gcmundmpr01qltmc10la0'
      }
    });
...
// app.get('*', async (req, res) => {console.log("ASDA")
// res.send("Hello")})

app.get('/api/historical_data', async (request, response) => {
  ...
  try {
    ...
    const res = await axios.get(`https://api.polygon.io/v2/aggs/ticker/${symbol}/range/1/day/${fromDate}/${today}?adjusted=true&sort=asc&apiKey=OpXGdu_iIjhzVRAXnby_ktJX7iHAurj9`);
